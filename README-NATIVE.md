# الربط مع تطبيق الأندرويد وتطبيق سطح المكتب (اختياري) — v1.12.0

## قسم «معلومات عن المدرسة»

الإصدار 1.12.0 يضيف هذا القسم داخل صفحة الويب نفسها، لذلك يعمل في WebView كما يعمل في المتصفح ولا يحتاج إلى تعديل Native كي يحفظ البيانات أو يطبعها أو ينشئ PDF. بيانات القسم مستقلة عن قاعدة بيانات الدرجات وتُحفظ محلياً في `localStorage` بالمفتاح `schoolResultsSchoolInfo_v1`. لا يدّعي هذا الإصدار مزامنة سحابية تلقائية من HTML فقط.

إذا كان تطبيق الأندرويد أو سطح المكتب يريد توفير حفظ/طباعة أصلية عبر النظام، يمكنه استخدام الجسر الاختياري الموصوف أدناه. لا تغيّر هذا الجسر بوابة تسجيل الدخول أو آلية التفعيل، ولا يحتاج الجهاز الملحق إلى تفعيل اشتراك جديد.
